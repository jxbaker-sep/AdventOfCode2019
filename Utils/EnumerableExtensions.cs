...
{
    public static IEnumerable<T> AllMinBy<T, K>(this IEnumerable<T> self, Func<T, K> predicate) where K: notnull
    {
        var groups = self.GroupBy(predicate).ToDictionary(it => it.Key, it => it.ToList());
        var key = groups.Keys.Min();
        return groups[key!];
    }
